...
def consultar_cita():
    ...
    with open(archivo_citas, "r", encoding="utf-8") as f:
        ...
        for fila in reader:
            if fila["Folio"] == folio:
                messagebox.showinfo("Consulta de cita",
                    f"Cita encontrada:\n\nNombre: {fila['Nombre']}\nCURP: {fila['CURP']}\n"
                    f"Trámite: {fila['Trámite']}\nFecha: {fila['Fecha']}\nHora: {fila['Hora']}")
                return
    messagebox.showwarning("No encontrada", f"No existe cita con folio {folio}.")
...
horas_disponibles = ["09:00 AM","09:30 AM","10:00 AM","10:30 AM","11:00 AM","11:30 AM","12:00 PM","12:30 PM"]
...
tk.Label(pagina_consulta, text="Ingrese el folio de la cita a consultar:", bg=color_content,
         fg="#333333", font=("Segoe UI", 16, "bold")).place(x=30, y=20)
